...
- 指针给了C++程序员很大的自由，但是也给了程序员很大的责任，指针可以指向任意内存地址，包括未分配的内存地址，
  这可能会导致程序崩溃或其他未定义的行为。
- C++创建指针时，计算机将分配内存空间来存储指针变量本身，但不负责分配指向的内存空间。
  - ```cpp
      int *p;
      *p = 123;
    ```
  - 上面的代码中，p是一个指针但是没有初始化，指向的内存地址是随机的，这个时候如果对p解引用，
    就会访问到随机的内存地址，这可能会导致程序崩溃或其他未定义的行为像这样的错误是极度危险的
    因此在使用 `*` 运算符解引用指针时，必须将指针初始化，指向一个确定的、合法内存地址。
- 指针提供了很大的灵活性C/C++程序员可以通过指针修改指定内存地址的值，但是一般情况下
  不建议直接修改指针指向的内存地址，因为这可能会导致程序崩溃或其他未定义的行为像 `init_ptr.cpp`的例子就是极度危险的，当然如果确认指针指向的内存地址是合法的，未被其他指针或变量占用，那么修改指针指向的内存地址是安全的，但是也需要谨慎操作。

#### 使用 `new` 来分配内存

- 变量是在$\color{skyblue}{编译时分配的由名称的内存}$,而指针是**存储内存地址的变量**，可以通过地址间接访问内存
- 指针的作用之一在与运行时分配为命名的内存，用于存储
- 在 C语言中可以通过 `malloc()` 函数来分配内存，在C++中也可以这么做
  - ```cpp
      malloc(sizeof(数据类型));
    ```
  - 上面的为 `malloc()` 函数的语法，
    其中 `sizeof(数据类型)` 用于计算数据类型的字节数
- 虽然C++中可以使用 `calloc()` 函数来分配内存,但是一般C++程序员不会使用 `calloc()`函数,
  因为有更好的选择 `new` 运算符:
  - `new` 会调用构造函数（对类类型）
  - `new` 类型更安全（不需要 `sizeof`）
  - `new` 与 `delete` 配对，语义更清晰
- `new` 运算符的语法为：
  - ```cpp
      new 数据类型;
      new 数据类型[数组大小];
    ```
- 分配内存给指针变量只是new运算符的一种用法，new运算符还可以用于分配数组内存、实例化对象等。
- 示例
  - ```cpp
      // use_new.cpp
      #include <iostream>
    ...
    - 运行成功输出结果为:
      - ```
          nighrts value = 1001: location 0x5ffe74
          int value = 1001: location 0x7408b0
          double value = 1001: location 0x7408f0
          location of pointer pt = 0x5ffe68
          size of pt = 8
          size of *pt = 4
          size of pd = 8
          size of *pd = 8
        ```
    - 程序说明
      - 上面的程序中，`new` 运算符用于分配内存给指针变量 `pt` 和 `pd`，
        并将指针变量初始化为指向分配的内存地址。
      - `*pt` 和 `*pd` 分别用于访问指针指向的内存地址中的值。
      - `sizeof(pt)` 和 `sizeof(*pt)` 分别用于计算指针变量 `pt` 和指针指向的内存地址中的值的字节数。
      - `sizeof(pd)` 和 `sizeof(*pd)` 分别用于计算指针变量 `pd` 和指针指向的内存地址中的值的字节数。
      - 上面的程序中，`sizeof(pt)` 用于计算指针变量 `pt` 的字节数，
        而 `sizeof(*pt)` 用于计算指针指向的内存地址中的值的字节数。
      - 类似的，`sizeof(pd)` 用于计算指针变量 `pd` 的字节数，
        而 `sizeof(*pd)` 用于计算指针指向的内存地址中的值的字节数。
      - 需要注意的是，`sizeof(pt)` 和 `sizeof(pd)` 的值通常是相同的，因为它们都是指针变量，占用的字节数取决于系统架构（如 32 位系统通常为 4 字节，64 位系统通常为 8 字节）。
- 使用 `new` 分配的内存和使用变量声明分配的内存是不同的，一般来说使用 `new` 分配的内存为 `堆内存`，而使用变量声明分配的内存为 `栈内存`。
- `堆内存` 和 `栈内存` 的区别
  - `栈内存` 是在程序运行时自动分配和释放的内存，通常用于存储局部变量和函数参数。
  - `堆内存` 是在程序运行时手动分配和释放的内存，通常用于存储动态分配的对象或数组。
  - |     特性     |         栈内存         |         堆内存         |
    | :----------: | :--------------------: | :--------------------: |
    |   分配方式   |     自动分配和释放     |     手动分配和释放     |
    |   存储内容   |   局部变量和函数参数   |  动态分配的对象或数组  |
    |   访问速度   |        快速访问        |        较慢访问        |
    | 内存大小限制 |   较小（通常几 MB）   |   较大（取决于系统）   |
    |   生命周期   | 随函数调用结束自动销毁 | 直到手动释放或程序结束 |

#### 使用 `delete` 来释放内存

- C语言和C++内存的管理是交给程序员的，程序员需要负责分配内存和释放内存。
- 在C语言中可以通过 `free()` 函数来释放内存，在C++中也可以这么做

  - ```cpp
      free(指针变量);
    ```
  - 上面的为 `free()` 函数的语法，其中 `指针变量` 是指向要释放内存的指针变量。
- 在C++中也可以使用 `delete` 运算符来释放内存，`delete` 运算符的语法为：

  - ```cpp
      delete 指针变量;
      delete[] 指针变量;
    ```
- `delete` 运算符用于释放单个对象的内存，而 `delete[]` 运算符用于释放数组的内存。
- 注意若是使用 `new` 运算符分配的内存必须使用 `delete` 运算符来释放，使用 `new[]` 运算符分配的内存必须使用 `delete[]` 运算符来释放。
- 在开发C和C ++程序遵循内存即用即分配的原则，即在需要使用内存时分配内存，在不需要使用内存时释放内存。
- 内存若是没有被释放就会造成内存泄漏，内存泄漏会导致程序占用的内存越来越多，最终可能会导致程序崩溃或系统崩溃。
- 使用 `delete` 释放内存时需要注意以下几点：

  - 只能释放使用 `new` 分配的内存，不能释放使用 `malloc()` 分配的内存。
  - 只能释放指针变量指向的内存，不能释放指针变量本身。
  - 只能释放一次同一块内存，不能重复释放同一块内存。
  - 释放内存后需要将指针变量设置为 `nullptr`，以避免悬空指针。
  - 释放前要确认这个指针不会被其他指针或变量使用，很多野指针错误就是因为释放了内存后没有将指针变量设置为 `nullptr`，导致其他指针或变量继续使用这个指针变量，最终导致程序崩溃或其他未定义的行为。如下面的代码就是一个典型的野指针错误
    - ```cpp
        int *p = new int(10); // 分配内存并初始化为 10
        delete p; // 释放内存
        // p 现在是一个悬空指针，指向已释放的内存
        std::cout << *p << std::endl; // 错误：访问已释放的内存，可能导致崩溃或未定义行为
      ```
    - 释放完内存后将指针变量设置为 `nullptr` 可以避免悬空指针错误，如下面的代码所示
      - ```cpp
          int *p = new int(10); // 分配内存并初始化为 10
          delete p; // 释放内存
          p = nullptr; // 将指针变量设置为 nullptr，避免悬空指针错误
          if (p) {
              std::cout << *p << std::endl; // 不会执行，因为 p 是 nullptr
          } else {
    ...
    - ```cpp
        ...
        - 运行成功输出结果为:
          - ```
              ...
              ```
##### 指针和字符串
- 数组和指针的关系可以拓展到C风格字符串，事实上在C语言中指针的最大作用之一就是处理字符串。
- C风格字符串是以空字符 `'\0'` 结尾的字符数组，指针可以用来访问和操作C风格字符串。
- 示例:
  - ```cpp
      // str_ptr3.cpp
      #include <iostream>
      ...
      - 运行成功输出结果为:
        - ```
            ...
            ```
- 注意：
  - 这里用的是字符串字面值，$\color{yellow}{字符串字面值在程序中是常量，存储在只读内存中，因此不能修改字符串字面值的内容}$。
  - 用char *str = "Hello";的好处是简单直接，节省内存空间。但是存在安全隐患，因为字符串字面值是只读的，如果尝试修改字符串内容会导致未定义行为。
  - 推荐的做法还是使用指针+动态内存分配的方法或者使用`string`类来处理字符串，这样更安全、更灵活。

##### 使用 `new` 创建动态结构

- 在运行时构建数组和结构要优于在编译时构建数组和结构，因为运行时构建可以根据需要分配内存，而编译时构建必须在编译时确定内存大小。
- 使用 `new` 运算符可以创建动态结构，语法如下：
  - ```cpp
      struct 结构体名 {
          成员变量类型 成员变量名;
          ...
      ...
      结构体名 *指针变量 = new 结构体名;
  ```
- 使用 `new` 创建动态结结构体时需要注意以下几点：

  - 释放动态结构体时必须使用 `delete` 运算符，而不是 `delete[]` 运算符。
  - 动态结构体的成员变量可以通过指针变量来访问，使用 `->` 运算符来访问成员变量。
  - 动态结构体的内存分配和释放必须成对出现，否则会导致内存泄漏或悬空指针错误。
- 只要使用指针来访问成员就不能使用点运算符 `.` 来访问成员变量，而必须使用箭头运算符 `->` 来访问成员变量。
- 示例:
  - ```cpp
      // use_new_struct.cpp
      #include <iostream>
  ...
  - ```cpp
      ...
      - 运行成功输出结果为:
        - ```
            ...
            ```
#### 模板类(array)
- 在`C++11` 中添加了模板类 `array` 模板类(array)是一个固定大小的数组，它可以存储任意类型的元素。
- 使用模板类(array)需要包含 `<array>` 头文件，如下所示：
  - ```cpp
      #include <array>
    ```
- 通过 `array` 类可以创建一个固定大小的数组，如下所示：
  - ```cpp
      std::array<数据类型, 数组大小> 变量名;
    ```
- `array` 类的常用方法有：
  - `size()`：返回数组的大小（元素个数）。
  - `empty()`：检查数组是否为空。。
...
- 示例：
  - ```cpp
      ...
      ```
#### `array` 和 `vector` 的区别
- `array` 是一个固定大小的数组，它的大小在编译时就确定了，不能动态改变。
- `vector` 是一个动态数组，它的大小可以在运行时改变。
- `array` 类的方法比较少，而 `vector` 类的方法比较多。
- `array` 类的元素存储在栈上，而 `vector` 类的元素存储在堆上。
- `array` 类的元素访问速度快，而 `vector` 类的元素访问速度慢。
- 若需要程度固定的数组，建议使用 `array` 类。

## 循环和关系表达式
### `for` 循环
- 在很多情况下程序需要处理重复的任务，例如遍历数组、计算总和等；而 `for` 循环可以帮助我们实现这些任务。
- `for` 循环的语法如下：
  - ```cpp
    ...
    ```
- `for` 循环的初始化表达式在循环开始前执行一次，用于初始化循环变量；表达式可以是一个$\color{yellow}{定义好并成功初始化的变量}$，也可以是一个初始化语句，例如 `int i = 0`。
- `条件循环` 是用于判断循环是否继续执行的表达式，若表达式结果为 `true` 则继续执行循环，若表达式结果为 `false` 则退出循环。
- `迭代表达式` 是在每次循环结束后执行的表达式，用于更新循环变量。
- `for` 循环为迭代循环，每次循环都会更新循环变量，直到循环条件为 `false` 时退出循环。
  ...
  - ```cpp
      ...
      - 运行成功输出结果为:
        - ```
            ...
            4
            ```
      - 这里的初始化表达式 `int i = 0` 用于初始化循环变量 `i`，将其赋值为 `0`。
      - 这里的循环条件 `i < 5` 用于判断循环是否继续执行，若 `i` 小于 `5` 则继续执行循环，否则退出循环。
      - 这里的迭代表达式 `i++` 用于更新循环变量 `i`，每次循环结束后将 `i` 增加 `1`,若这里是 `++i` 则是先将 `i` 增加 `1` 再使用 `i`。
      - 这里的循环体 `cout << i << endl;` 用于输出循环变量 `i` 的值。
- 示例2:
  - ```cpp
      // for_example2.cpp
      #include <iostream>
      ...
      - 运行成功输出结果为:
        - ```
            ...
            4
            ```
      - 这里的初始化表达式是一个定义好的并成功初始化的变量
      - 这里的循环条件是 `i < sizeof(arr)/sizeof(arr[0])`
        - `sizeof(arr)` 是数组占用的内存大小
        - `sizeof(arr[0])` 是数组中每个元素占用的内存大小，
